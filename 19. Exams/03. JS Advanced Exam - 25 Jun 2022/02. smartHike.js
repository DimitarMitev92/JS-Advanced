class SmartHike {
    ...
    showRecord(criteria) {
        ...
        if (criteria === 'hard') {
            let bestHike = this.listOfHikes.filter(hike => hike.difficultyLevel === 'hard').sort((a, b) => a.time - b.time)[0];
            ...
            let bestHike = this.listOfHikes.filter(hike => hike.difficultyLevel === 'easy').sort((a, b) => a.time - b.time)[0];
            if (bestHike === undefined) {
                return `${this.username} has not done any ${criteria} hiking yet`;
            } else {
                return `${this.username}'s best ${criteria} hike is ${bestHike.peak} peak, for ${bestHike.time} hours`;
